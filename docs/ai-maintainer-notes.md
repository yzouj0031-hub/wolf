...
## 12. AI 文案的"每晚循环 vs 整局一次"歧义

给 AI 看的技能说明里，**"仅一次"必须写清作用域**。曾出现蚀时狼妃反弹写"首个命中触发，仅一次"，
AI 理解成"整局用一次就废"，后续夜晚不再锁定——实际是每晚循环技（开关每夜随 `S.nightData` 重置）。
**规则**：循环技一律写"当晚/本夜只 X 一次；每晚可重新使用，非整局一次性"；真正的一次性技才写"整局仅一次"。
凡改到技能说明，desc / guide / 每晚 prompt / 私密记忆 **四处措辞要一致**（AI 会读全部）。

## 13. 新增"干扰类"效果，记得同步"受干扰方的意识提示"

子狐封技/禁刀、蚀时狼妃反弹会干扰别人的夜间技能。对应有两段意识注入（`WB_TAMPER_GODS` 给好人神职、
`WB_TAMPER_WOLVES` 给会刀的狼）。它们的触发条件**必须覆盖积木自创角色的对应块**（`reflect`/`muteSkill`/`blockKill`），
不能只判 reg 的子狐/蚀时狼妃；受提示方也要包含**积木自创的好人神职**（customAbilities 含 inspect/protect/poison/save/shield），
否则自创干扰角色在场时，被干扰的一方拿不到"你的结果未必可靠"的提醒。
