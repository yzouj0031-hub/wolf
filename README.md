# 🐺 AI 狼人杀 · Multi-Agent Werewolf

**一个让不同大模型自主开局、互相推理与欺骗的社会推理沙盒 —— 同时也是一款可以自己上手玩的狼人杀。**

> A sandbox for watching different LLMs play social deduction against each other — they keep secrets, lie, form alliances, and vote each other out. Also a fully playable Werewolf game.

GPT / Gemini / Claude / DeepSeek / Qwen / Grok 等模型各自扮演角色，从第一晚的狼人密谋，一路推理、撒谎、结盟、投票，直到分出胜负。你可以纯观战，也可以自己加入一局。

## ▶ 30 秒看一局（无需任何 API key）

**[点这里打开一局自动回放 → yzouj0031-hub.github.io/wolf/?demo=1](https://yzouj0031-hub.github.io/wolf/?demo=1)**

一局**真实抓取**的对局会在牌桌上自动播放：`Gemini 2.5 Pro`、`GPT-5`、`Claude 4 Sonnet`、`Gemini 2.5 Flash` 等模型分饰狼人、白猫、狼美人、村民等角色。你会看到狼人在夜间私聊里商量刀谁、好人白天如何抓逻辑漏洞、身份如何被一步步撕开。**不用填 key，不用等模型生成，点开即看。**

也可以直接在大厅点 **「🎬 观战 Demo」** 按钮。

## 你会看到什么

- **私有信息与公开博弈分离**：狼人有独立的夜间密聊，预言家/女巫等有私密信息；模型必须在只掌握自己那份信息的前提下发言。
- **真实的欺骗与推理**：狼人伪装身份、悍跳、倒钩；好人交叉验证发言、找票型矛盾。
- **多模型同场**：同一局里不同厂商的模型对垒，能直接观察谁更会骗、谁更容易被骗。
- **完整技能体系**：预言家、女巫、猎人、守卫、骑士等多种角色与自定义技能。

## 作为「AI 社会推理实验」

即便只有一个人跑，这个环境也能持续产出有意思的观察对象：

- 模型能否在多轮里**守住身份秘密**、会不会**误泄私有信息**
- 能否**识别谎言**、会不会**形成联盟**
- 不同模型的**欺骗能力差异**、长上下文后推理**是否退化**
- 模型是否**遵守角色约束**、同一模型**当狼 vs 当好人**的策略差异

### 研究记录（进行中）

下面这张表是自动锦标赛的目标格式。目前**大规模、多模型的自动对局与回归测试主要受限于 API 推理成本**，所以真实数据还在小样本积累中：

| Model | 好人胜率 | 狼人胜率 | 规则违规 | 平均 token |
| --- | ---: | ---: | ---: | ---: |
| GPT | — | — | — | — |
| Claude | — | — | — | — |
| Gemini | — | — | — | — |

> 一条命令跑 N 局 → 自动生成对局 JSON → 胜率 / token / 违规次数 → 可回放网页，是接下来的方向；算力到位后即可批量填表。

## 自带模型 / BYOK

游戏本体不替你烧钱：**每个 AI 角色都可以单独配置接口地址、密钥和模型名**。只要是 **OpenAI 兼容格式**的接口都能接，因此 OpenAI / Gemini（兼容代理）/ Anthropic（兼容代理）/ 各类中转与自建 endpoint 都可以直接用。代码里已对 GPT / Gemini / DeepSeek / Qwen / Grok / MiniMax 等模型的思考字段（thinking / reasoning）做了适配。

> 注意：游戏只支持 OpenAI 兼容格式，各家**原生**接口需要通过兼容代理地址接入。

## 其他能力

- 简体中文 / English 界面切换，英文规则导出与英文 AI 行动指令
- 存档、复盘与观战体验
- 桌面端、移动端与 PWA 离线访问
