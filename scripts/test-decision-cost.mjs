...
for (const file of FILES) {
  ...
  // ★ 被说服之后要能写出他付了什么代价。
  // 两种完全不同的东西会产生一模一样的"我信了"：他做了对狼很贵的事 / 他只是说得顺。
  // 从感受上分不开——AI 尤其分不开，它读到流畅自洽的发言产生的"可信"感，
  // 和读到真实代价产生的"可信"感是同一个东西。所以必须要求它写出代价。
  assert.match(c, /【被说服之后，写得出他付了什么代价吗】/, `${file}: 缺少"被说服要能写出代价"`);
  assert.match(c, /一种是他做了一件对狼来说很贵的事，另一种只是他把话说得顺/, `${file}: 没有点出两种说服来源`);
  assert.match(c, /说不出来，那你是被文笔说服的/, `${file}: 没有给出可执行的自检`);
  assert.match(c, /诚恳、卖惨、态度配合、逻辑顺畅全都零成本/, `${file}: 没有把"逻辑顺畅"也归为零成本`);
